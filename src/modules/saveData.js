const saveData = (projectsArray) => {
    ...
    projectsArray.forEach((project, projectIndex) => {
        ...
        project.getTodoArray().forEach((todo, todoIndex) => {
            localStorage.setItem(`todo-${todoIndex}`, JSON.stringify({'parentProjectId': `${projectIndex}`, 'todoId': todoIndex, 'name': todo.getName(), 'description': todo.getDescription(), 'due-date': todo.getDueDate(), 'priority': todo.getPriority()}));
        });
    });
}
